...
import { railsUrl, basicHeader, pickCookie, extractCsrf, SIGNIN_PATH } from '../rails/_utils';
...
export async function POST(req: Request) {
  const { email, password, csrf: csrfFromClient } = await req.json() as {
    email?: string; password?: string; csrf?: string;
  };
  if (!email || !password) {
    return NextResponse.json({ error: 'email and password required' }, { status: 400 });
  }

  // Read our browser-side mirror of Rails session
  const store = await cookies();
  let railsSession = store.get('rails_session')?.value ?? null;

  // If no CSRF provided, fetch sign-in page now (and refresh session if rotated)
  ...
  if (!csrf) {
    const pre = await fetch(railsUrl(SIGNIN_PATH), {
      ...
      headers: {
        ...
        ...(railsSession ? { Cookie: `_mercato_session=${railsSession}` } : {}),
      },
      cache: 'no-store',
      redirect: 'manual',
    });
  ...
  if (!railsSession) {
    return NextResponse.json({ error: 'Rails session unavailable (call /api/auth/csrf first)' }, { status: 428 });
  }

  // POST Devise HTML form
  ...
  if (!me.ok) {
    ...
    profileError.cookies.set(ACCOUNT_ID_COOKIE, '', { ...cookieOptions, maxAge: 0 });
    return profileError;
  }
  ...
  if (typeof accountId !== 'number' && typeof accountId !== 'string') {
    ...
    invalidRes.cookies.set(ACCOUNT_ID_COOKIE, '', { ...cookieOptions, maxAge: 0 });
    return invalidRes;
  }
